...
// import { CRMIconFont } from '@/components/CRMIconFont';
import { Button, Form, Input, Select, DatePicker } from 'antd';
// import styles from '../clients/ClientList.less';
...
const SearchForm: React.FC<{ searchFormConfig: any[]; resetForm: any; searchForm: any }> = ({
  ...
  const [clientSearchForm] = Form.useForm(); // 定义表单实例
  ...
  const toResetForm = useCallback(() => {
    clientSearchForm.resetFields();
    resetForm();
  }, [clientSearchForm, resetForm]);

  // 查询列表数据
  const toSearch = () => {
    const formValues = clientSearchForm.getFieldsValue();
      ...
      >
        {searchFormConfig.map((item) => {
          if (item.itemType === 'empty') {
            // 空的填充元素
            return <div className={styles.formItem} key={item.name}></div>;
          }
          return (
            <Form.Item
        ...
        {<div className={styles.formItem} key={'empty'}></div>}
